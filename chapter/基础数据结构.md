...
​         Vec是一个模板类，主要用于存储数值向量。编译期几经知道大小，称为固定类向量。固定类向量不涉及动态内存分配，程序执行速度快。
...
vectors are allocated on stack, as opposite to std::valarray, std::vector, cv::Mat etc., which elements are dynamically allocated in the heap.
...
RotatedRect该类表示平面上的旋转矩形，该类对象有三个重要属性：矩形中心点(质心)，边长(长和宽)，旋转角度。
...
>
> The template class is similar and compatible with std::complex, however it provides slightly   more convenient access to the real and imaginary parts using through the simple field access, as opposite
